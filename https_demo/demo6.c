...
static int usage(void)
{
  ...
  printf("--time_inc increate N secords for OCSP RESPONSE valid time.\n");
  printf("-v Enable verbose.\n");
  printf("--runtime the running time.\n");
  exit(-1);
}

static void stats(int sig){
  fprintf(stderr, "succeed (%ld) failed(%ld) connections.\n", i, j);
  exit(0);
}
...
int main(int argc, char ** argv)
{
  ...
  const char *opt_url = NULL, *url = NULL;
  long time_inc = 0;
  ...
  while(argc > 0){
    if(strcmp(*argv,"--url") == 0){
      if(--argc<1)
        break;
      opt_url = *(++argv);
    }else if(strcmp(*argv,"--time_inc") == 0 ){
      if(--argc<1)
        break;
      time_inc = atol(*(++argv));
  ...
  if(curl) {
    ...
    do { 
      ...
      /** */
      curl_easy_setopt(curl, CURLOPT_SSL_VERIFYSTATUS_INC_OCSPSTAPLING_VALID, time_inc);
      /** 打印错误信息 */
      ...
      if(res != CURLE_OK) {
        ...
        fprintf(stderr,"succeed (%lu), failed (%lu)\n", i, j);
